 ...
 * @{
 * @addtogroup Naze64 Tau Labs Naze64 support files
 * @{
 *
 * @file       pios_board.c
...
static const struct pios_exti_cfg pios_exti_mpu6000_cfg __exti_config = {
	...
	.irq = {
		.init = {
			...
			.NVIC_IRQChannelPreemptionPriority = PIOS_IRQ_PRIO_HIGH, /* TODO MID? */
...
void PIOS_Board_Init(void) {
	...
	uintptr_t flash_id;
	if (PIOS_Flash_Jedec_Init(&flash_id, pios_spi_internal_id, 3, &flash_w25x_cfg) != 0)
		panic(4);
	if (PIOS_FLASHFS_Logfs_Init(&pios_uavo_settings_fs_id, &flashfs_settings_w25x_cfg, &pios_jedec_flash_driver, flash_id) != 0)
		panic(4);
	if (PIOS_FLASHFS_Logfs_Init(&pios_waypoints_settings_fs_id, &flashfs_waypoints_w25x_cfg, &pios_jedec_flash_driver, flash_id) != 0)
		panic(4);
#endif
	...
	if (boot_count < 3) {
		...
		HwNaze64SetDefaults(HwNaze64Handle(), 0);
	...
	HwNaze64USB_VCPPortGet(&hw_usb_vcpport);

	if (!usb_cdc_present) {
		/* Force VCP port function to disabled if we haven't advertised VCP in our USB descriptor */
		hw_usb_vcpport = HWNAZE64_USB_VCPPORT_DISABLED;
	}

	switch (hw_usb_vcpport) {
	case HWNAZE64_USB_VCPPORT_DISABLED:
		break;
	case HWNAZE64_USB_VCPPORT_USBTELEMETRY:
		...
		break;
	case HWNAZE64_USB_VCPPORT_COMBRIDGE:
#if defined(PIOS_INCLUDE_COM)
		{
			uint32_t pios_usb_cdc_id;
			if (PIOS_USB_CDC_Init(&pios_usb_cdc_id, &pios_usb_cdc_cfg, pios_usb_id)) {
...
#endif	/* PIOS_INCLUDE_COM */
		break;
	case HWNAZE64_USB_VCPPORT_DEBUGCONSOLE:
#if defined(PIOS_INCLUDE_COM)
#if defined(PIOS_INCLUDE_DEBUG_CONSOLE)
		{
			uint32_t pios_usb_cdc_id;
	...
	HwNaze64USB_HIDPortGet(&hw_usb_hidport);

	if (!usb_hid_present) {
		/* Force HID port function to disabled if we haven't advertised HID in our USB descriptor */
		hw_usb_hidport = HWNAZE64_USB_HIDPORT_DISABLED;
	}

	switch (hw_usb_hidport) {
	case HWNAZE64_USB_HIDPORT_DISABLED:
		break;
	case HWNAZE64_USB_HIDPORT_USBTELEMETRY:
#if defined(PIOS_INCLUDE_COM)
		{
	...
	uint8_t hw_DSMxBind;
	HwNaze64DSMxBindGet(&hw_DSMxBind);

	/* UART1 Port */
	uint8_t hw_flexi;
	HwNaze64FlexiPortGet(&hw_flexi);
	switch (hw_flexi) {
	case HWNAZE64_FLEXIPORT_DISABLED:
		break;
	case HWNAZE64_FLEXIPORT_TELEMETRY:
...
#endif
		break;
	case HWNAZE64_FLEXIPORT_DEBUGCONSOLE:
...
#endif	/* PIOS_INCLUDE_DEBUG_CONSOLE */
		break;
	case HWNAZE64_FLEXIPORT_COMBRIDGE:
#if defined(PIOS_INCLUDE_USART) && defined(PIOS_INCLUDE_COM)
...
#endif
		break;
	case HWNAZE64_FLEXIPORT_MAVLINKTX:
		...
		break;
	case HWNAZE64_FLEXIPORT_MAVLINKTX_GPS_RX:
	...
	/* UART2 Port */
	uint8_t hw_main;
	HwNaze64MainPortGet(&hw_main);
	switch (hw_main) {
	case HWNAZE64_MAINPORT_DISABLED:
		break;
	case HWNAZE64_MAINPORT_TELEMETRY:
#if defined(PIOS_INCLUDE_TELEMETRY_RF) && defined(PIOS_INCLUDE_USART) && defined(PIOS_INCLUDE_COM)
	...
	uint8_t hw_rcvrport;
	HwNaze64RcvrPortGet(&hw_rcvrport);

	switch (hw_rcvrport) {
	case HWNAZE64_RCVRPORT_DISABLED:
		break;
	case HWNAZE64_RCVRPORT_PWM:
	...
	case HWNAZE64_RCVRPORT_PPM:
		...
		break;
	case HWNAZE64_RCVRPORT_DSM2:
	case HWNAZE64_RCVRPORT_DSMX10BIT:
	case HWNAZE64_RCVRPORT_DSMX11BIT:
	...
	HwNaze64MPU6000DLPFGet(&hw_mpu6000_dlpf);
	enum pios_mpu60x0_filter mpu6000_dlpf = \
	    (hw_mpu6000_dlpf == HWNAZE64_MPU6000DLPF_256) ? PIOS_MPU60X0_LOWPASS_256_HZ : \
	    (hw_mpu6000_dlpf == HWNAZE64_MPU6000DLPF_188) ? PIOS_MPU60X0_LOWPASS_188_HZ : \
	    (hw_mpu6000_dlpf == HWNAZE64_MPU6000DLPF_98) ? PIOS_MPU60X0_LOWPASS_98_HZ : \
	    (hw_mpu6000_dlpf == HWNAZE64_MPU6000DLPF_42) ? PIOS_MPU60X0_LOWPASS_42_HZ : \
	    (hw_mpu6000_dlpf == HWNAZE64_MPU6000DLPF_20) ? PIOS_MPU60X0_LOWPASS_20_HZ : \
	    (hw_mpu6000_dlpf == HWNAZE64_MPU6000DLPF_10) ? PIOS_MPU60X0_LOWPASS_10_HZ : \
	    (hw_mpu6000_dlpf == HWNAZE64_MPU6000DLPF_5) ? PIOS_MPU60X0_LOWPASS_5_HZ : \
	    pios_mpu6000_cfg.default_filter;
	PIOS_MPU6000_SetLPF(mpu6000_dlpf);

	uint8_t hw_mpu6000_samplerate;
	HwNaze64MPU6000RateGet(&hw_mpu6000_samplerate);
	uint16_t mpu6000_samplerate = \
	    (hw_mpu6000_samplerate == HWNAZE64_MPU6000RATE_200) ? 200 : \
	    (hw_mpu6000_samplerate == HWNAZE64_MPU6000RATE_333) ? 333 : \
	    (hw_mpu6000_samplerate == HWNAZE64_MPU6000RATE_500) ? 500 : \
	    (hw_mpu6000_samplerate == HWNAZE64_MPU6000RATE_666) ? 666 : \
	    (hw_mpu6000_samplerate == HWNAZE64_MPU6000RATE_1000) ? 1000 : \
	    (hw_mpu6000_samplerate == HWNAZE64_MPU6000RATE_2000) ? 2000 : \
	    (hw_mpu6000_samplerate == HWNAZE64_MPU6000RATE_4000) ? 4000 : \
	    (hw_mpu6000_samplerate == HWNAZE64_MPU6000RATE_8000) ? 8000 : \
...
#if defined(PIOS_INCLUDE_HMC5983)
	PIOS_HMC5983_Init(pios_spi_internal_id, 1, &pios_hmc5983_cfg);
	PIOS_MPU6000_SetSampleRate(mpu6000_samplerate);
	PIOS_WDG_Clear();
...
#if defined(PIOS_INCLUDE_MS5611_SPI)
	PIOS_MS5611_Init(pios_spi_internal_id, 2, &pios_ms5611_cfg);
