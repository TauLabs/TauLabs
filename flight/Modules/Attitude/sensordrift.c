 ...
 */
void CottonComplementaryCorrection(float *accels, float *gyros,
				   const float delT)
{
	float grav_b[3];
	float accel_err_b[3];
	...
	grav_b[2] =
	    -(glbl->q[0] * glbl->q[0] - glbl->q[1] * glbl->q[1] -
	      glbl->q[2] * glbl->q[2] + glbl->q[3] * glbl->q[3]);
	...
	// Accumulate integral of error.  Scale here so that units are (deg/s) but
	// accelKi has units of s
	glbl->gyro_correct_int[0] += accel_err_b[0] * glbl->accelKi;
	glbl->gyro_correct_int[1] += accel_err_b[1] * glbl->accelKi;

	// Because most crafts wont get enough information from gravity to zero yaw
	// gyro, we try and make it average zero (weakly)
	glbl->gyro_correct_int[2] += -gyros[2] * glbl->yawBiasRate;

	// In this step, correct rates based on proportional error. The integral
	// component is applied in updateSensors
 ...
 */
void DcmCorrection(float *accels, float *gyros, float Rbe[3][3],
		   const float delT, bool GPS_Drift_Compensation)
{
...
//      dcmStatus.rawGyros[2]=gyros[2];
	...
	//The math is derived from Roll-Pitch Gyro Drift Compensation, Rev.3, by W.
	//Premerlani
#if defined (PIOS_INCLUDE_GPS)
	if (drft->gpsPresent_flag && GPS_Drift_Compensation) {
		float accels_e[3];

		//Rotate accelerometer readings into Earth frame. Note that we need to
		//take the transpose of Rbe.
	...
	if (!GPS_Drift_Compensation) {
		...
		{
			//We're actually using new GPS data here, but it's already been
			//stored in old by the previous function
			yaw_drift_MagGPS(Rbe, true, drft->magNewData_flag,
					 errYaw_b);
			...
			//In addition to calculating the roll-pitch-yaw error, we can
			//calculate yaw drift, errYaw_b, based on GPS and attitude data

			//We're actually using new GPS data here, but it's already been
			//stored in old by the previous function
			yaw_drift_MagGPS(Rbe, false, drft->magNewData_flag,
					 errYaw_b);
		}
...
#endif
		//In addition, we can calculate roll-pitch error with only the aid of
		//an accelerometer
	...
	gyro_drift(gyros, errYaw_b, errRollPitch_b, normOmegaScalar, delT,
		   omegaCorrP, drft->omegaCorrI);

	calibrate_gyros_high_speed(gyros, omegaCorrP, normOmegaScalar, delT,
				   glbl->gyroGain);
	...
	//Add 0.0001% of proportional error back into gyroscope bias offset. This
	//keeps DC elements out of the raw gyroscope data.
...
/*
 * Calculate the error, as indicated by the accelerometer. It's important to
 * use the true airspeed here, and not CAS or EAS
 */
void rollPitch_drift_accel(float accels[3], float gyros[3], float Rbe[3][3],
			   float airspeed_tas, float *errRollPitch_b)
{
	...
	//Combine measurements and centripetal accelerations. This should give the
	//gravity reference vector
	//Remember that for fixed-wing, acc_centripetal[0] = 0
	g_ref[0] = accels[0];
	...
	float normG =
	    sqrtf(g_ref[0] * g_ref[0] + g_ref[1] * g_ref[1] +
		  g_ref[2] * g_ref[2]);
	g_ref[0] /= normG;
	g_ref[1] /= normG;
	g_ref[2] /= normG;

	//Error is cross product of reference vector and estimated orientation.
	//Reverse operation in order to have correct sign for application to error
	//sum
	CrossProduct((const float *)fuselageZ, (const float *)g_ref,
		     errAccelsRollPitch_b);
...
/*
 * This function takes in a number of different error rotations, weights them,
 * and applies them to the gyroscope readings. In addition, there is a function
 * to increase the correction at high speeds in order to keep the gyro from
 * becoming "dizzy".
 */
void gyro_drift(float gyro[3], float errYaw_b[3], float errRollPitch_b[3],
		float normOmegaScalar, float delT, float *omegaCorrP,
		float *omegaCorrI)
{
	...
	// boost the KPs at high spin rate, to compensate for increased error due
	// to calibration error above 50 degrees/second, scale by rotation rate
	// divided by 50. As per Fast Rotations, by William Premerlani. 
	...
	// Compute integral correction. Turn off the offset integrator while
	// spinning at high speeds, it doesn't work in that case, and it only
	// causes trouble.
	if (normOmegaScalar < MAXIMUM_SPIN_DCM_INTEGRAL) {
		omegaCorrI[0] +=
		    (errRollPitch_b[0] * drft->rollPitchKi +
		     errYaw_b[0] * drft->yawKi) * delT;
		omegaCorrI[1] +=
		    (errRollPitch_b[1] * drft->rollPitchKi +
		     errYaw_b[1] * drft->yawKi) * delT;
		omegaCorrI[2] +=
		    (errRollPitch_b[2] * drft->rollPitchKi +
		     errYaw_b[2] * drft->yawKi) * delT;
	}
}
 ...
 */
void rollPitch_drift_GPS(float Rbe[3][3], float accels_e_int[3],
			 float delT_between_updates, float *errRollPitch_b)
{
	...
	dGPSdt_e[0] =
	    (gpsVelocity.North - drft->GPSV_old[0]) / delT_between_updates;
	dGPSdt_e[1] =
	    (gpsVelocity.East - drft->GPSV_old[1]) / delT_between_updates;
	dGPSdt_e[2] =
	    GRAV + (gpsVelocity.Down -
		    drft->GPSV_old[2]) / delT_between_updates;
	...
	//Take cross product of integrated accelerometer measurements with
	//integrated earth frame accelerations. We should be using normalized
	//dGPSdt, but we perform that calculation in the following line(s).
	CrossProduct((const float *)accels_e_int, (const float *)dGPSdt_e,
		     errRollPitch_e);
				...
				float normOmegaScalar, float delT, float *ggain)
{
	if (normOmegaScalar > MINIMUM_SPIN_RATE_GYRO_CALIB) {
		float normOmegaVector[3] =
		    { gyro[0] / normOmegaScalar, gyro[1] / normOmegaScalar,
			gyro[2] / normOmegaScalar
		};

		//Calculate delta gain and update gains
		ggain[0] +=
		    normOmegaVector[0] * omegaCorrP[0] / normOmegaScalar *
		    (glbl->gyroGain_ref / drft->gyroCalibTau) * delT;
		ggain[1] +=
		    normOmegaVector[1] * omegaCorrP[1] / normOmegaScalar *
		    (glbl->gyroGain_ref / drft->gyroCalibTau) * delT;
		ggain[2] +=
		    normOmegaVector[2] * omegaCorrP[2] / normOmegaScalar *
		    (glbl->gyroGain_ref / drft->gyroCalibTau) * delT;
		...
		for (int i = 0; i < 3; i++) {
			...
			ggain[i] =
			    ggain[i] > highThresh ? highThresh : ggain[i];
		}
	}
}
		      ...
		      bool magNewData_flag, float *errYaw_b)
{
	...
	//Define forward vector in earth frame, Rbe' * [1;0;0], and eliminate
	//vertical component from vector
	...
	if (gpsNewData_flag) {
		...
		if (fabs(gpsVelocity.North) > GPS_SPEED_MIN
			...
			// However, save several processor cycles by condensing the math to
			// take advantage of the cross products null entries on the x and y
			// elements. errorGPSYaw_e = horizDirOverGndRmat X
			// horizDirOverGndGPS
			errorGPSYaw_e =
			    horizDirOverGndRmat[0] * horizDirOverGndGPS[1] -
			    horizDirOverGndGPS[0] * horizDirOverGndRmat[1];

			// Rotate error to body frame. Again, take advantage of the yaw
			// error vector [0;0;errorGPSYaw_e]'s null entries
...
//THIS PIOS DEFINE NOT CURRENTLY EXIST, BUT WE SHOULD ADD IT IN ORDER TO SUPPORT ALL MAGS, NOT JUST THE HMC5883
#if defined (PIOS_INCLUDE_MAGNETOMETER)
	if (magNewData_flag) {
		...
		float mags_b[3] = { magnetometerData.x, magnetometerData.y,
			magnetometerData.z
		};
		...
		// However, save several processor cycles by condensing the math to
		// take advantage of the cross products null entries on the x and y
		// elements. errorMagYaw_e = horizDirOverGndRmat X horizDirOverGndMag
		errorMagYaw_e =
		    horizDirOverGndRmat[0] * horizDirOverGndMag[1] -
		    horizDirOverGndMag[0] * horizDirOverGndRmat[1];

		// Rotate error to body frame. Again, take advantage of the yaw error
		// vector [0;0;errorGPSYaw_e]'s null entries errorMagYaw_b = Rbe *
		// errorMagYaw_e;
