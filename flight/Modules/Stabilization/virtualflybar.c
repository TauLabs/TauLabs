...
int stabilization_virtual_flybar(float gyro, float command, float *output, float dT, bool reinit, uint32_t axis, struct pid *pid, float scale, StabilizationSettingsData *settings)
{
	...
	float pid_out = pid_apply_setpoint(pid,  0,  gyro, dT, scale);
