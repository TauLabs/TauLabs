...
static double TranMerc_Origin_int = 0.0;          /* longitude of origin in radians */
...
{ /* BEGIN Set_Tranverse_Mercator_Parameters */
  ...
  { /* no errors */
    ...
    TranMerc_Origin_int = Central_Meridian;
    ...
    Convert_Geodetic_To_Transverse_Mercator(MAX_LAT,
                                            MAX_DELTA_int + Central_Meridian,
...
{ /* BEGIN Get_Tranverse_Mercator_Parameters  */
  ...
  *Central_Meridian = TranMerc_Origin_int;
...
{      /* BEGIN Convert_Geodetic_To_Transverse_Mercator */
  ...
  if (longitude > PI)
    longitude -= (2 * PI);
  if ((longitude < (TranMerc_Origin_int - MAX_DELTA_int))
      || (longitude > (TranMerc_Origin_int + MAX_DELTA_int)))
  {
    if (longitude < 0)
      temp_int = longitude + 2 * PI;
    else
      temp_int = longitude;
    if (TranMerc_Origin_int < 0)
      temp_Origin = TranMerc_Origin_int + 2 * PI;
    else
      temp_Origin = TranMerc_Origin_int;
    if ((temp_int < (temp_Origin - MAX_DELTA_int))
        || (temp_int > (temp_Origin + MAX_DELTA_int)))
  ...
  { /* no errors */
     ...
     *  Delta longitude
     */
    dlam = longitude - TranMerc_Origin_int;
...
{      /* BEGIN Convert_Transverse_Mercator_To_Geodetic */
  ...
  if (!Error_Code)
  {
    ...
    *Latitude = ftphi - pow(de,2) * t10 + pow(de,4) * t11 - pow(de,6) * t12 
    ...
    /* longitude */
    (*longitude) = TranMerc_Origin_int + dlam;
