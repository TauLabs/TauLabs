 ...
 * @{
 * @addtogroup PIOS_MS5803 MS5803 Functions
 * @brief Hardware functions to deal with the altitude pressure sensor
 * @{
 *
 * @file       pios_ms5803_spi.c
 * @author     Kenn Sebesta, Copyright (C) 2016
 * @brief      MS5803 Pressure Sensor Routines
 *
 ******************************************************************************/
...
#if defined(PIOS_INCLUDE_MS5803_SPI) || 1
...
#define MS5803_TASK_PRIORITY	PIOS_THREAD_PRIO_HIGHEST
#define MS5803_TASK_STACK_BYTES	512

/* MS5803 Addresses */
#define MS5803_RESET            0x1E
#define MS5803_CALIB_ADDR       0xA2  /* First sample is factory stuff */
#define MS5803_CALIB_LEN        16
#define MS5803_ADC_READ         0x00
#define MS5803_PRES_ADDR        0x40
#define MS5803_TEMP_ADDR        0x50
#define MS5803_ADC_MSB          0xF6
#define MS5803_P0               101.3250f

/* Private methods */
static int32_t PIOS_MS5803_Read(uint8_t address, uint8_t * buffer, uint8_t len);
static int32_t PIOS_MS5803_WriteCommand(uint8_t command);
static void PIOS_MS5803_Task(void *parameters);

/* Private types */

/* Local Types */

enum pios_ms5803_dev_magic {
        PIOS_MS5803_DEV_MAGIC = 0x778FBD1A, // md5 of "pios_ms5803_spi"
};
...
struct ms5803_dev {
        const struct pios_ms5803_cfg *cfg;
	uint32_t spi_id;
	uint32_t slave_num;
	...
	enum conversion_type current_conversion_type;
        enum pios_ms5803_dev_magic magic;

	struct pios_semaphore *busy;
};

static struct ms5803_dev *dev;

/**
 * @brief Allocate a new device
 */
static struct ms5803_dev * PIOS_MS5803_alloc(void)
{
        struct ms5803_dev *ms5803_dev;

        ms5803_dev = (struct ms5803_dev *)PIOS_malloc(sizeof(*ms5803_dev));
        if (!ms5803_dev)
		return (NULL);

        memset(ms5803_dev, 0, sizeof(*ms5803_dev));

        ms5803_dev->queue = PIOS_Queue_Create(1, sizeof(struct pios_sensor_baro_data));
        if (ms5803_dev->queue == NULL) {
                PIOS_free(ms5803_dev);
		return NULL;
	}

        ms5803_dev->magic = PIOS_MS5803_DEV_MAGIC;

        ms5803_dev->busy = PIOS_Semaphore_Create();
        PIOS_Assert(ms5803_dev->busy != NULL);
 ...
 */
static int32_t PIOS_MS5803_Validate(struct ms5803_dev *dev)
{
	if (dev == NULL)
		return -1;
        if (dev->magic != PIOS_MS5803_DEV_MAGIC)
		return -2;
	if (dev->spi_id == 0)
		return -3;
	return 0;
...
/**
 * Initialise the MS5803 sensor
 */
int32_t PIOS_MS5803_SPI_Init(uint32_t spi_id, uint32_t slave_num, const struct pios_ms5803_cfg *cfg)
{
        dev = (struct ms5803_dev *)PIOS_MS5803_alloc();
	if (dev == NULL)
		return -1;
	...
	dev->cfg = cfg;
        ...
        if (PIOS_MS5803_WriteCommand(MS5611_RESET) != 0)
		return -2;
	...
	for (int i = 0; i < NELEMENTS(dev->calibration); i++) {
                PIOS_MS5803_Read(MS5611_CALIB_ADDR + i * 2, data, 2);
	...
	dev->task = PIOS_Thread_Create(
                        PIOS_MS5803_Task, "pios_ms5803", MS5803_TASK_STACK_BYTES, NULL, MS5803_TASK_PRIORITY);
...
/**
 * Claim the MS5803 device semaphore
 * \return 0 if no error
 * \return -1 if timeout before claiming semaphore
 */
static int32_t PIOS_MS5803_ClaimDevice(void)
{
        PIOS_Assert(PIOS_MS5803_Validate(dev) == 0);
 ...
 */
static int32_t PIOS_MS5803_ReleaseDevice(void)
{
        PIOS_Assert(PIOS_MS5803_Validate(dev) == 0);
 ...
 */
static int32_t PIOS_MS5803_ClaimBus(void)
{
        if (PIOS_MS5803_Validate(dev) != 0)
		return -1;
 ...
 * @return 0 if successful
 */
static int32_t PIOS_MS5803_ReleaseBus(void)
{
        if (PIOS_MS5803_Validate(dev) != 0)
		return -1;
...
static int32_t PIOS_MS5611_StartADC(enum conversion_type type)
{
	...
	case TEMPERATURE_CONV:
                while (PIOS_MS5803_WriteCommand(MS5611_TEMP_ADDR + dev->cfg->oversampling) != 0)
			continue;
		break;
	case PRESSURE_CONV:
                while (PIOS_MS5803_WriteCommand(MS5611_PRES_ADDR + dev->cfg->oversampling) != 0)
			continue;
		break;
	default:
		return -1;
...
*/
static int32_t PIOS_MS5803_ReadADC(void)
{
        if (PIOS_MS5803_Validate(dev) != 0)
		return -1;
	...
	if (dev->current_conversion_type == TEMPERATURE_CONV) {
		...
		/* Read the temperature conversion */
                if (PIOS_MS5803_Read(MS5611_ADC_READ, data, 3) != 0)
			return -1;
...
out:
        PIOS_MS5803_ReleaseBus();

	return rc;
}

/**
* Writes one or more bytes to the MS5803
...
*/
int32_t PIOS_MS5803_SPI_Test()
{
        if (PIOS_MS5803_Validate(dev) != 0)
		return -1;


        PIOS_MS5803_ClaimDevice();
	PIOS_MS5611_StartADC(TEMPERATURE_CONV);
	PIOS_DELAY_WaitmS(PIOS_MS5611_GetDelay());
        PIOS_MS5803_ReadADC();
        PIOS_MS5803_ReleaseDevice();
	...
	while (1) {
		...
		if (temp_press_interleave_count == 0)
		{
			...
			PIOS_Thread_Sleep(PIOS_MS5611_GetDelay());
                        read_adc_result = PIOS_MS5803_ReadADC();
                        PIOS_MS5803_ReleaseDevice();
