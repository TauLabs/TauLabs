...
for i=1:N
    for j=i:N
        if P(i,j) == 0
            ...
            Pstrings{i,j} = strrep(Pstrings{i,j},'(Ts + e_tau)^3','Ts_e_tau2');
        ...
        s1 = sprintf('P[%d][%d] = ',i-1,j-1);
        s2 = sprintf('P[%d][%d] = ',j-1,i-1);
        if (i==j)
            Pstrings{i,j} = [s1, Pstrings{i,j}, ';'];
        else
            Pstrings{i,j} = [s1, s2, Pstrings{i,j}, ';'];
        end
    end
end
...
for k = N:-1:1
    ...
    for i=1:N
        for j=i:N
            Pstrings{i,j} = strrep(Pstrings{i,j},s1,s2);
        end
    end
end
...
for i=1:N;
    for(j=i:N);
        nummults=nummults + length(strfind(Pstrings{i,j},'*'));
        numadds=numadds + length(strfind(Pstrings{i,j},'+'));
        numadds=numadds + length(strfind(Pstrings{i,j},'-'));
    end
end
nummults
...
for i=1:N
    ...
    Pstrings2{i} = strrep(Pstrings2{i},'(Ts + e_tau)^2','Ts_e_tau2');
        for n1 = 1:N
            for n2 = 1:N
                Pstrings2{i} = strrep(Pstrings2{i},sprintf('P_%d_%d^2',n1,n2),sprintf('P_%d_%d*P_%d_%d',n1,n2,n1,n2));
            end
        end
    s1 = sprintf('X[%d] = ',i-1);
