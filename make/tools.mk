...
# Set up QT toolchain
QT_SDK_DIR := $(TOOLS_DIR)/Qt5.2.0-beta1
...
ifdef LINUX
  ifdef AMD64
    # Linux 64-bit
    qt_sdk_install: QT_SDK_URL := http://download.qt-project.org/development_releases/qt/5.2/5.2.0-beta1/qt-linux-opensource-5.2.0-beta1-x86_64-offline.run
    QT_SDK_QMAKE_PATH := $(QT_SDK_DIR)/5.2.0-beta1/gcc_64/bin/qmake
  else
    # Linux 32-bit
    qt_sdk_install: QT_SDK_URL := http://download.qt-project.org/development_releases/qt/5.2/5.2.0-beta1/qt-linux-opensource-5.2.0-beta1-x86-offline.run
    QT_SDK_QMAKE_PATH := $(QT_SDK_DIR)/5.2.0-beta1/gcc/bin/qmake
  endif
endif

ifdef MACOSX
  qt_sdk_install: QT_SDK_URL  := http://download.qt-project.org/development_releases/qt/5.2/5.2.0-beta1/qt-mac-opensource-5.2.0-beta1-clang-offline.dmg
  QT_SDK_QMAKE_PATH := $(QT_SDK_DIR)/5.2.0-beta1/clang_64/bin/qmake
endif

ifdef WINDOWS
  qt_sdk_install: QT_SDK_URL  := http://download.qt-project.org/development_releases/qt/5.2/5.2.0-beta1/qt-windows-opensource-5.2.0-beta1-mingw48_opengl-x86-offline.exe 
  QT_SDK_QMAKE_PATH := $(QT_SDK_DIR)/5.2.0-beta1/mingw48_32/bin/qmake
endif
...
ifneq (,$(filter $(UNAME), Darwin))
	...
	$(V1) /tmp/qt-installer/qt-mac-opensource-5.2.0-beta1-clang-offline.app/Contents/MacOS/qt-mac-opensource-5.2.0-beta1-clang-offline 
...
ifneq (,$(filter $(UNAME), MINGW32_NT-6.1))
	$(V1) ./downloads/qt-windows-opensource-5.2.0-beta1-mingw48_32-x86-offline.exe
endif
