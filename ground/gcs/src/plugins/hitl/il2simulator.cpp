...
void IL2Simulator::processUpdate(const QByteArray& inp)
{
    ...
    if (std::isnan(old.ddX) || std::isinf(old.ddX)) old.ddX=0;
    if (std::isnan(old.ddY) || std::isinf(old.ddY)) old.ddY=0;
    if (std::isnan(old.ddZ) || std::isinf(old.ddZ)) old.ddZ=0;
...
#define TURN_FILTER 10
    ...
    if (std::isnan(old.dAzimuth) || std::isinf(old.dAzimuth)) old.dAzimuth=0;
    if (std::isnan(old.dPitch)   || std::isinf(old.dPitch)) old.dPitch=0;
    if (std::isnan(old.dRoll)    || std::isinf(old.dRoll)) old.dRoll=0;
